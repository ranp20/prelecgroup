    ...
    <div class="col-xl-9 col-lg-8 order-lg-2">
      <div class="card blog-details-box">
        <div class="blog-details-slider owl-carousel">
          ...
          <img src="{{asset('assets/images/blogs/'.$photo)}}" alt="Image">
          @endforeach
        </div>
      ...
      <div class="resent-blog-slider owl-carousel" >
        ...
        <div class="widget widget-featured-posts">
          <div class="entry">
            <div class="entry-thumb"><a href="{{route('front.blog.details',$like_post->slug)}}"><img src="{{asset('assets/images/blogs/'.json_decode($like_post->photo,true)[array_key_first(json_decode($like_post->photo,true))])}}" alt="Post"></a></div>
            <div class="entry-content">
              <h4 class="entry-title">
                <a href="{{route('front.blog.details',$like_post->slug)}}">{{ strlen(strip_tags($like_post->title)) > 75 ? substr(strip_tags($like_post->title), 0, 75) . '...' : strip_tags($like_post->title) }}</a>
              </h4>
        ...
        <div class="entry">
          <div class="entry-thumb">
            <a href="{{route('front.blog.details',$recent->slug)}}">
              <img src="{{ asset('assets/images/blogs/'.json_decode($recent->photo,true)[array_key_first(json_decode($recent->photo,true))]) }}" alt="Post">
            </a>
          </div>
          <div class="entry-content">
            <h4 class="entry-title">
              <a href="{{route('front.blog.details',$recent->slug)}}">{{ strlen(strip_tags($recent->title)) > 55 ? substr(strip_tags($recent->title), 0, 55) . '...' : strip_tags($recent->title) }}</a>
            </h4>
            <span class="entry-meta">{{__('by')}} {{__('Admin')}}</span>
          </div>
        </div>
